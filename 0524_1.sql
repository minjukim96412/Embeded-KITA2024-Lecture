...
-- 사용자 소유의 테이블 정보를 가진 USER_TABLES;
SELECT TABLE_NAME FROM USER_TABLES;
SELECT * FROM USER_TABLES;

--사용자 소유의 테이블과 사용자가 조희가능한 테이블명을 조회
SELECT  TABLE_NAME FROM ALL_TABLES;
SELECT * FROM  ALL_TABLES;

--DBA_USER 라는 관리자가 볼 수 있는 사용자 정보사전
...
--인덱스를 사용해서 인덱스가 없을 때의 검색속도보다 더 떨어진다면 인덱스를 만들지 않는 것이 좋다.
--다양한 컬럼이나 컬럼조합에 인덱스를 생성해서 검색속도를 체크해 보는것이 현명하다.
...
-- TOP-N QUERY (조회결과 행의 상단 몇개만 추출하는 쿼리, 페이징 할 때 많이 사용)
...
MAXVALUE 20 -- 최대값
MINVALUE 10 -- 최소값
CYCLE -- MAXVALUE에 도달했을때 다시 처음부터 시작할지
...
CREATE TABLE CONS_TEST (
	...
	CTSID VARCHAR(20) CONSTRAINT CONS_UQ UNIQUE
);
...
DELETE  FROM CONS_TEST WH CTNO =2;
