...
public class StringExer {
	public static void main(String[] args) {
		...
		//인덱스 = 0~ length-1
		String str3 = "hello Java";//index 0~9, length10
...
//		System.out.println(str4);
		...
		int i;
		//i는 main메소드 내에서 선언된 지역변수이므로 main메소드 밖에서는 사용할 수 있다.
		//i는 main메소드의 지역변수 이므로 초기화 후에만 사용 할 수 있다.
//		System.out.println("선언 후 : " + i);
		...
		//문자열들이 메모리에 생성됨
		//StringBuffer또는 StringBuilder를 사용해서 문자열 조작작업을
		...
		System.out.println(str10.contains("백두산"));// true 포함하고있는지?
		...
		String str11 = "    헬로우    ";
 		...
 		//실습 1)exerStr 문자열의 양쪽 공백 제거
 		System.out.println(exerStr.trim());
 		//실습 2)Time을 Gold로 변경
 		System.out.println(exerStr.replace("Time","Gold"));
 		//실습 3)전화번호를 010-1234-4678 로 변경해서 전화번호만 출력
 		String[] exerSplit3 = exerStr.split(" ");
 		for(String str : exerSplit3) {
 			if(str.length() >=13) {
 				System.out.println(str.replaceAll("/", "-"));
 				}
 			}
 		
 		System.out.println(exerStr.substring(exerStr.indexOf("0"), 
 				exerStr.indexOf("0")+13).replace("/", "-"));
 		
 		//실습 4)ExerStr문자열을 공백 기준으로 분리하여 String[]로 변환해 출력
 		String[] exerSplit = exerStr.split(" ");
 		for(String str : exerSplit) {
 			System.out.println(str);
 		}
 		//실습 5)exerStr문자열을 공백 기준으로 분리하여 문자수가 3이상인 것들만 출력
 		String[] exerSplit2 = exerStr.split(" ");
 		for(String str : exerSplit2) {
 			if(str.length() >=3) {
 				System.out.println(str);
 				}
 			}
